...
static int updateStart(char *statusdir_name)
{
    int ret;
    ...
    /*  Perform the start-update logic. */
    BUMStateNext_StartUpdate(BUM_state_p);
    /*  Save the BUM state */
...
int main(int argc, char** argv)
{
    ...
    if(argc != 2){
        ...
        ret = updateStart(argv[1]);
        if(0 != ret)
            fprintf(stderr, "   updateStart failed\n");
    }
    return ret;
}
